...
//包含了配置日志记录器的各种选项，如日志文件目录、应用程序名称、
//错误日志文件名、警告日志文件名等。
type Options struct {
	LogFileDir    string
	AppName       string
...
var (
	...
	debugConsoleWS                 = zapcore.Lock(os.Stdout) // 控制台标准输出 锁机制
...
func (l *Logger) cores() zap.Option {
	...
	return zap.WrapCore(func(c zapcore.Core) zapcore.Core {
		//apcore.NewTee 函数能够确保日志消息会同时传递给每个核心组件，从而实现多重日志输出的功能。
